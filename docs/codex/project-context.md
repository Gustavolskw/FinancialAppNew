...
Este arquivo agora está na raiz do monorepo AppFinancasNew. A documentação abaixo preserva
o contexto detalhado do backend porque ele continua sendo o módulo mais maduro do projeto.
Para trabalhos iniciados na raiz, considere também:
...
O backend mantém o padrão controller fino -> `ActionManager` -> `Action` -> Configuration configurável
...
Aplicação React Router 7, React 19, TypeScript, Vite e Tailwind. O frontend ainda está no template inicial e deve evoluir para telas reais do AppFinancasNew. A documentação específica fica em `../frontEnd/docs/codex`.

### `../docker-compose.yml`

Compose da raiz. Hoje sobe PostgreSQL, backend e frontend em modo desenvolvimento. Detalhes ficam em `docs/codex/docker.md`.
