...
use App\Infrastructure\DTO\Configuration\ExpenseConfiguration;
...
final class ExpenseController extends AbstractController
{
    ...
    public function list(Request $request, #[MapQueryString] ExpenseQueryParamsDto $queryDto, EntityManagerInterface $entityManager): JsonResponse
    {
        return $this->actionManager
            ->handle(ExpenseConfiguration::build($entityManager), $request, QueryParams::fromArray($queryDto->toArray()))
            ->output();
    }
    ...
    public function byWallet(int $walletId, Request $request, EntityManagerInterface $entityManager): JsonResponse
    {
        ...
        return $this->actionManager
            ->handle(ExpenseConfiguration::build($entityManager), $request, QueryParams::fromArray($queryParams))
            ->output();
    }
    ...
    {
        return $this->actionManager
            ->handle(ExpenseConfiguration::build($entityManager), $request, id: $id)
            ->output();
    }
    ...
    {
        return $this->actionManager
            ->handle(ExpenseConfiguration::build($entityManager), $request, formDto: $formDto)
            ->output();
    }
